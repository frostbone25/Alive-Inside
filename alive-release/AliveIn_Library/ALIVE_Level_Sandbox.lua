...
local kScript = "ALIVE_Level_Sandbox"; --dont touch (the name of this script and also the name of the level function at the bottom of the script, which will be called as soon as this scene opens)
local kScene = "adv_boardingSchoolExterior"; --dont touch (the name of the scene asset file)

--scene agent name variable
local agent_name_scene = "adv_boardingSchoolExterior.scene"; --dont touch (this is the name of the scene agent object, using it to set post processing effects later)

--cutscene development variables variables (these are variables required by the development scripts)
ALIVE_Development_SceneObject = kScene; --dont touch (the development scripts need to reference the main level)
ALIVE_Development_SceneObjectAgentName = agent_name_scene; --dont touch (the development scripts also need to reference the name of the scene agent)
ALIVE_Development_UseSeasonOneAPI = false; --dont touch (this is leftover but if the development tools were implemented inside season 1 we need to use the S1 functions because the api changes)
ALIVE_Development_FreecamUseFOVScale = false; --changes the camera zooming from modifing the FOV directly, to modifying just the FOV scalar (only useful if for some reason the main field of view property is chorelocked or something like that)

--dof autofocus variables
ALIVE_DOF_AUTOFOCUS_SceneObject = kScene; --dont touch (the dof autofocus script need to reference the main level)
ALIVE_DOF_AUTOFOCUS_SceneObjectAgentName = agent_name_scene; --dont touch
ALIVE_DOF_AUTOFOCUS_UseCameraDOF = true; --uses the depth of field properties on the camera rather than on the scene post processing.
...
Scene_RelightScene = function()
    ...
    ALIVE_AgentSetProperty("myLight_Sun", "EnvLight - Intensity", 6, kScene);
...
local agent_name_thirdPersonGroupCamera = "myThirdPersonGroupCamera";

local clemAnimCnt_idle = nil;
local clemAnimCnt_walking = nil;
local clemAnimCnt_running = nil;
local clemAnimCnt_crouchIdle = nil;
local clemAnimCnt_crouchMoving = nil;

Cutscene_CreateThirdPersonController = function()
    local cam_prop = "module_camera.prop";

    local newPosition = Vector(0,15,0);
    local newRotation = Vector(90,0,0);

    local cameraParentAgent = AgentCreate(agent_name_thirdPersonGroupCamera, "group.prop", newPosition, newRotation, kScene, false, false);
    local cameraAgent = AgentCreate(agent_name_thirdPersonCamera, cam_prop, newPosition, newRotation, kScene, false, false);

    ALIVE_AgentSetProperty(agent_name_thirdPersonCamera, "Clip Plane - Far", 2500, kScene);
