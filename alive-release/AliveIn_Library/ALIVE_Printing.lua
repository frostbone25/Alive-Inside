...
--[[
AgentSetState = function(agent, stateName)
  -- function num : 0_9 , upvalues : _ENV
  local agentProps = AgentGetRuntimeProperties(agent)
  if not PropertyHasGlobal(agentProps, "module_agent_state") then
    return 
  end
  local stateProps = ContainerGetElement(PropertyGet(agentProps, "States"), stateName)
  if not stateProps then
    return 
  end
  for _,key in ipairs(PropertyGetKeys(stateProps)) do
    if PropertyExists(agentProps, key) then
      PropertySet(agentProps, key, PropertyGet(stateProps, key))
    end
  end
end
]]--
