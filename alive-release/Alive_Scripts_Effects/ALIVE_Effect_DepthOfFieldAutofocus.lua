...
- ALIVE_DOF_AUTOFOCUS_SceneObject
...
- ALIVE_DOF_AUTOFOCUS_SceneObjectAgentName
...
- ALIVE_DOF_AUTOFOCUS_UseCameraDOF
...
Callback_OnPostUpdate:Add(ALIVE_Effect_DepthOfFieldAutofocus_PerformAutofocus)
]]--
...
ALIVE_Effect_DepthOfFieldAutofocus_SetupDOF = function(kScene)
...
ALIVE_Effect_DepthOfFieldAutofocus_PerformAutofocus = function()
    ...
    local currentCamera_agent = SceneGetCamera(ALIVE_DOF_AUTOFOCUS_SceneObject); --Agent type
    ...
    local currentCamera_far_plane = ALIVE_AgentGetProperty(currentCamera_name, "Clip Plane - Far", ALIVE_DOF_AUTOFOCUS_SceneObject); --Number type
    ...
    local rejectionDistance = 40.0; --the max distance before the agent is too far from camera to do autofocus
    
    --list of valid target agents
    local validAgentsArray = {}; --Agent array
    
    --find all of the valid target objects in the scene
    for i, objectName in ipairs(ALIVE_DOF_AUTOFOCUS_ObjectEntries) do
        ...
        object_agent = AgentFindInScene(objectName, ALIVE_DOF_AUTOFOCUS_SceneObject); --Agent type
        
        --if the object exists
        if (object_agent) then
            --assign our variables
            object_isOnScreen = AgentIsOnScreen(object_agent);
            object_isVisible = ALIVE_AgentGetProperty(objectName, "Runtime: Visible", ALIVE_DOF_AUTOFOCUS_SceneObject);
    ...
    for x, validAgent in ipairs(validAgentsArray) do
        ...
        if (nearFocusTarget1_agent) then
            ...
            nearFocusTarget1_agent = ALIVE_GetNearestAgent(currentCamera_agent, nearFocusTarget1_agent, validAgent);
        else
        ...
        if (farFocusTarget2_agent) then
            ...
            farFocusTarget2_agent = ALIVE_GetFarthestAgent(currentCamera_agent, farFocusTarget2_agent, validAgent);
        else
    ...
    for i, name in ipairs(ALIVE_DOF_AUTOFOCUS_GameplayCameraNames) do
        --if the current camera is infact a gameplay camera
        if (currentCamera_name == ALIVE_DOF_AUTOFOCUS_GameplayCameraNames[i]) then
    ...
    if (ALIVE_DOF_AUTOFOCUS_UseCameraDOF == true) then
        ...
        --Use the DOF properties on the scene post processing itself
        ALIVE_AgentSetProperty(ALIVE_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Enabled", current_enabledDOF, ALIVE_DOF_AUTOFOCUS_SceneObject)
